...
// Onyx's white QR square is baked into the same snippet as the logo; the editor draws its own
// rounded white panel, so cut the square out and leave the logo alone.
{
  const f = path.join(ASSET_DIR, 'onix-main-deco2.svg');
  const svg = readFileSync(f, 'utf-8');
  const cut = svg.replace(/<path class="st1" d="M115\.59,123\.35[^>]*><\/path>/, '');
  if (cut === svg) throw new Error('Onyx white square not found in deco2');
  writeFileSync(f, cut, 'utf-8');
}

// Same for the stickers: their white QR square (M203.14,191.44…) duplicates the editor's own panel.
for (const [file, cls] of [
  ['naklejka-white-main-decoX1.svg', 'st5'],
  ['naklejka-black-main-decoX2.svg', 'st4'],
]) {
  const f = path.join(ASSET_DIR, file);
  const svg = readFileSync(f, 'utf-8');
  const cut = svg.replace(new RegExp(`<path class="${cls}" d="M203\\.14,191\\.44[^>]*></path>`), '');
  if (cut === svg) throw new Error(`Sticker white square not found in ${file}`);
  writeFileSync(f, cut, 'utf-8');
}
